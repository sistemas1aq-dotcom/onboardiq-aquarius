  ...
  template: `
    <div>
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Gestion de Cargos</h1>
      </div>

      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100 flex items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            <select [(ngModel)]="filterAreaId" (ngModelChange)="page = 1" class="px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20">
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr *ngFor="let c of paginatedCargos; let i = index" class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2">
                    <button (click)="openModal(c)" class="text-blue-600 hover:text-blue-800 text-xs font-medium">Editar</button>
              ...
              <tr *ngIf="filteredCargos.length === 0">
                <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-400">No hay cargos registrados</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div *ngIf="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
          <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h3 class="text-lg font-semibold text-gray-900">{{ editing ? 'Editar Cargo' : 'Nuevo Cargo' }}</h3>
            <button (click)="modalOpen = false" class="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
          </div>
          <div class="px-6 py-4 space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Nombre *</label>
              <input type="text" [(ngModel)]="form.nombre" placeholder="Nombre del cargo" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"/>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Area *</label>
              <select [(ngModel)]="form.area_id" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20">
...
export class CargosComponent implements OnInit {
  ...
  modalOpen = false;
  editing: any = null;
  form: any = { nombre: '', area_id: null, descripcion: '' };
  ...
  get filteredCargos(): any[] {
    if (!this.filterAreaId) return this.cargos;
    return this.cargos.filter((c) => c.area_id === this.filterAreaId);
  }
  ...
  openModal(cargo?: any): void {
    this.editing = cargo || null;
    this.form = cargo
  ...
  save(): void {
    if (!this.form.nombre || !this.form.area_id) { this.showError('Nombre y area son obligatorios'); return; }
    this.saving = true;
    const obs = this.editing
      ? this.http.put(`${this.apiUrl}/maestros/cargos/${this.editing.id}`, this.form)
      : this.http.post(`${this.apiUrl}/maestros/cargos`, this.form);
    obs.subscribe({
      next: () => {
        this.modalOpen = false;
        this.saving = false;
        this.showSuccess(this.editing ? 'Cargo actualizado' : 'Cargo creado');
        this.loadCargos();
      },
      error: (err: any) => {
        this.saving = false;
